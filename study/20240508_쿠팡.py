...
# 상품 정보 수집
products = []
for soup in page_link:
    time.sleep(1)
    # soup = load_source(link)
    # products_list = soup.find_allsoup.find_all(class_="baby-product renew-badge ")
    for product_info in products_list:
        product = {}
        product["img_src"] = product_info.img.get("src")
        product["link_addr"] = product_info.find(class_="baby-product-link").get("href")
        product["product_name"] = product_info.find(class_="name").text
        products.append(product)
